...
class Kernel:
    ...
    def split_train_val(self, X, Y, val_fraction=0.1, batch_size=5)-> None:
        ...
        self.train_loader=torch.utils.data.DataLoader([[X[i], Y[i]] for i in range(N_train)], batch_size=batch_size, shuffle=True)  
        self.val_loader=torch.utils.data.DataLoader([[X[i], Y[i]] for i in range(N_train, N)], batch_size=batch_size, shuffle=True)
