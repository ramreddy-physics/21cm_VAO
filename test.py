...
Model=torch.load('UNet_vcb.pt', map_location=torch.device('cpu'))

vcb_pred=np.reshape(Model(X).detach().numpy(), (N_test, 16, 16, 16))

from sklearn.metrics import mean_squared_error as mse

print('mean squared error on testing dataset = %f'%mse(vcb.reshape(N_test, 16**3), vcb_pred.reshape(N_test, 16**3))) #between original and reconstructed